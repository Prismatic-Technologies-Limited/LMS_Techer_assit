...
class ChatRequest(BaseModel):
    message: str
    profile_url: str
    instructor_id: int   # ✅ all required now

class ChatResponse(BaseModel):
    response: str

# Initialize Groq client
groq_client = Groq(api_key=os.getenv("GROQ_API_KEY", "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx"))
...
async def chat_with_teacher_assistant(request: ChatRequest):
    ...
    if not request.message or not request.profile_url or not request.instructor_id:
        raise HTTPException(status_code=400, detail="message, profile_url, and instructor_id are required")

    instructor_data = await fetch_instructor_profile(request.profile_url)
    bot_response = await generate_chat_response(request.message, request.profile_url, instructor_data)
